...
import { FaArrowLeft, FaAppleAlt, FaHistory, FaDownload, FaPrint, FaShare, FaTrash } from 'react-icons/fa';
...
export default function MacroCalculator() {
  ...
  useEffect(() => {
    if (history.length > 0) localStorage.setItem('macro_history', JSON.stringify(history));
  }, [history]);
  ...
  const calculateMacros = () => {
    if (!calories) {
      showToast('Please enter your daily calories', 'error');
      return;
    }
    ...
    const resultData = {
      ...
      fat: Math.round(fat),
      goal: goal === 'maintain' ? 'Maintain Weight' : goal === 'bulk' ? 'Build Muscle' : 'Lose Weight',
  ...
  const resetCalculator = () => { setCalories(''); setResult(null); showToast('Reset', 'info'); };
  const clearHistory = () => { setHistory([]); localStorage.removeItem('macro_history'); showToast('History cleared', 'info'); };
          ...
          <div style={{ background: 'linear-gradient(135deg, #059669 0%, #047857 50%, #065f46 100%)', borderRadius: '16px', padding: '32px 36px', marginBottom: '32px', position: 'relative', overflow: 'hidden', color: 'white', boxShadow: '0 4px 20px rgba(5, 150, 105, 0.25)' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div>
                  <h1 style={{ fontSize: '1.8rem', fontWeight: '800', margin: 0, letterSpacing: '-0.5px' }}>Macro Calculator</h1>
                  <p style={{ fontSize: '0.9rem', opacity: 0.9, margin: '2px 0 0', fontWeight: '300' }}>Calculate your ideal macronutrient split</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px' }}>
            <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: '1px solid #e2e8f0' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <input type="number" value={calories} onChange={e => setCalories(e.target.value)} placeholder="Daily Calories *" style={{ width: '100%', padding: '12px 16px', borderRadius: '12px', border: '1.5px solid #e2e8f0', fontSize: '1rem', outline: 'none' }} />
                <div style={{ display: 'flex', gap: '8px' }}>
                  {['maintain', 'bulk', 'lose'].map(g => (
                    <button key={g} onClick={() => setGoal(g)} style={{ flex: 1, padding: '10px', borderRadius: '10px', border: `2px solid ${goal === g ? emerald : '#e2e8f0'}`, background: goal === g ? '#ecfdf5' : 'white', fontWeight: '700', cursor: 'pointer' }}>
                      {g === 'maintain' ? '⚖️ Maintain' : g === 'bulk' ? '💪 Bulk' : '🔥 Lose'}
                    </button>
                  ))}
                </div>
                <div style={{ display: 'flex', gap: '12px' }}>
                  <button onClick={calculateMacros} style={{ flex: 2, padding: '14px', background: emerald, color: 'white', border: 'none', borderRadius: '12px', fontWeight: '700', fontSize: '1rem', cursor: 'pointer' }}>Calculate</button>
            ...
            <div>
              {result ? (
                <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: `2px solid ${emerald}` }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '16px' }}>
                    <div style={{ textAlign: 'center', background: '#fef2f2', padding: '16px', borderRadius: '12px' }}>
                      ...
                      <div style={{ fontSize: '1.8rem', fontWeight: '800', color: '#ef4444' }}>{result.protein}g</div>
                    </div>
                    <div style={{ textAlign: 'center', background: '#fef3c7', padding: '16px', borderRadius: '12px' }}>
                      ...
                      <div style={{ fontSize: '1.8rem', fontWeight: '800', color: '#d97706' }}>{result.carbs}g</div>
                    </div>
                    <div style={{ textAlign: 'center', background: '#ecfdf5', padding: '16px', borderRadius: '12px' }}>
                      ...
                      <div style={{ fontSize: '1.8rem', fontWeight: '800', color: '#059669' }}>{result.fat}g</div>
                    </div>
                  </div>
                  <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '10px' }}>
                    <div style={{ fontSize: '0.85rem', color: '#1e293b' }}><strong>Goal:</strong> {result.goal} • <strong>Calories:</strong> {result.calories} kcal</div>
                  </div>
                </div>
              ) : (
                <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '300px' }}>
                  ...
                  <p style={{ color: '#94a3b8', textAlign: 'center' }}>Enter your calories to calculate macros</p>
                </div>
              )}
            </div>
          </div>
