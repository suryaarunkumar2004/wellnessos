...
export default function WaterIntakeCalculator() {
  ...
  const activityLevels = {
    ...
    moderate: { label: 'Moderate Exercise', multiplier: 1.2 },
    active: { label: 'Active', multiplier: 1.4 },
    very_active: { label: 'Very Active', multiplier: 1.6 }
  };
  ...
  useEffect(() => {
    if (history.length > 0) localStorage.setItem('water_history', JSON.stringify(history));
  }, [history]);
  ...
  const calculateWater = () => {
    ...
    const multiplier = activityLevels[activity].multiplier;
    const water = Math.round(base * multiplier * 1000);
    const glasses = Math.round(water / 250);
    const bottles = Math.round(water / 500);
    ...
    const resultData = {
      ...
      weight: w,
      gender,
      activity: activityLevels[activity].label,
  ...
  const resetCalculator = () => { setWeight(''); setResult(null); showToast('Reset', 'info'); };
  const clearHistory = () => { setHistory([]); localStorage.removeItem('water_history'); showToast('History cleared', 'info'); };
          ...
          <div style={{ background: 'linear-gradient(135deg, #059669 0%, #047857 50%, #065f46 100%)', borderRadius: '16px', padding: '32px 36px', marginBottom: '32px', position: 'relative', overflow: 'hidden', color: 'white', boxShadow: '0 4px 20px rgba(5, 150, 105, 0.25)' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div>
                  <h1 style={{ fontSize: '1.8rem', fontWeight: '800', margin: 0, letterSpacing: '-0.5px' }}>Water Intake Calculator</h1>
                  <p style={{ fontSize: '0.9rem', opacity: 0.9, margin: '2px 0 0', fontWeight: '300' }}>Calculate your daily hydration needs</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px' }}>
            <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: '1px solid #e2e8f0' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <select value={activity} onChange={e => setActivity(e.target.value)} style={{ width: '100%', padding: '12px 16px', borderRadius: '12px', border: '1.5px solid #e2e8f0', fontSize: '1rem', outline: 'none', background: 'white' }}>
                  {Object.entries(activityLevels).map(([key, val]) => <option key={key} value={key}>{val.label}</option>)}
                </select>
                <div style={{ display: 'flex', gap: '12px' }}>
                  <button onClick={calculateWater} style={{ flex: 2, padding: '14px', background: emerald, color: 'white', border: 'none', borderRadius: '12px', fontWeight: '700', fontSize: '1rem', cursor: 'pointer' }}>Calculate</button>
              ...
              {result ? (
                <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: `2px solid ${emerald}` }}>
                  ...
                  <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                    <div style={{ fontSize: '3rem', fontWeight: '800', color: '#06b6d4' }}>{result.water} ml</div>
                    <div style={{ fontSize: '1rem', color: '#64748b' }}>Daily Water Intake</div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
                    <div style={{ textAlign: 'center', background: '#f8fafc', padding: '12px', borderRadius: '10px' }}>
                      <div style={{ fontSize: '0.6rem', color: '#94a3b8' }}>Glasses</div>
                      <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#1e293b' }}>{result.glasses} glasses</div>
                    </div>
                    <div style={{ textAlign: 'center', background: '#f8fafc', padding: '12px', borderRadius: '10px' }}>
                      <div style={{ fontSize: '0.6rem', color: '#94a3b8' }}>Bottles</div>
                      <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#1e293b' }}>{result.bottles} bottles</div>
                    </div>
                  </div>
                </div>
              ) : (
                <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '300px' }}>
                  ...
                  <p style={{ color: '#94a3b8', textAlign: 'center' }}>Enter your weight to calculate water needs</p>
                </div>
              )}
            </div>
          </div>
