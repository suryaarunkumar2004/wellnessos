...
export default function RunningPaceCalculator() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('pace_history');
    if (saved) setHistory(JSON.parse(saved));
  }, []);

  useEffect(() => {
    if (history.length > 0) localStorage.setItem('pace_history', JSON.stringify(history));
  }, [history]);
  ...
  const calculatePace = () => {
    if (!distance || !time) {
      showToast('Please enter both distance and time', 'error');
      return;
    }
    ...
    showToast('✅ Pace calculated!', 'success');
  };

  const resetCalculator = () => { setDistance(''); setTime(''); setResult(null); showToast('Reset', 'info'); };
  const clearHistory = () => { setHistory([]); localStorage.removeItem('pace_history'); showToast('History cleared', 'info'); };
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 24px' }}>
          ...
          <div style={{ background: 'linear-gradient(135deg, #059669 0%, #047857 50%, #065f46 100%)', borderRadius: '16px', padding: '32px 36px', marginBottom: '32px', position: 'relative', overflow: 'hidden', color: 'white', boxShadow: '0 4px 20px rgba(5, 150, 105, 0.25)' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div>
                  <h1 style={{ fontSize: '1.8rem', fontWeight: '800', margin: 0, letterSpacing: '-0.5px' }}>Running Pace Calculator</h1>
                  <p style={{ fontSize: '0.9rem', opacity: 0.9, margin: '2px 0 0', fontWeight: '300' }}>Pace, time & distance from any two inputs</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px' }}>
            <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: '1px solid #e2e8f0' }}>
              <h3 style={{ fontSize: '1.2rem', fontWeight: '800', color: '#1e293b', marginBottom: '20px' }}>Enter Your Run Details</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <input type="number" step="0.1" value={distance} onChange={e => setDistance(e.target.value)} placeholder="Distance (km) *" style={{ width: '100%', padding: '12px 16px', borderRadius: '12px', border: '1.5px solid #e2e8f0', fontSize: '1rem', outline: 'none' }} />
                <input type="number" value={time} onChange={e => setTime(e.target.value)} placeholder="Time (minutes) *" style={{ width: '100%', padding: '12px 16px', borderRadius: '12px', border: '1.5px solid #e2e8f0', fontSize: '1rem', outline: 'none' }} />
                <div style={{ display: 'flex', gap: '12px' }}>
                  <button onClick={calculatePace} style={{ flex: 2, padding: '14px', background: emerald, color: 'white', border: 'none', borderRadius: '12px', fontWeight: '700', fontSize: '1rem', cursor: 'pointer' }}>Calculate Pace</button>
            ...
            <div>
              {result ? (
                <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: `2px solid ${emerald}` }}>
                  <h3 style={{ fontSize: '1.2rem', fontWeight: '800', color: '#1e293b', marginBottom: '16px' }}>Your Pace</h3>
                  <div style={{ textAlign: 'center', marginBottom: '16px' }}>
                    <div style={{ fontSize: '3rem', fontWeight: '800', color: emerald }}>{result.pace}</div>
                    <div style={{ fontSize: '1rem', color: '#64748b' }}>Average Pace</div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                    <div style={{ textAlign: 'center', background: '#f8fafc', padding: '16px', borderRadius: '12px' }}>
                      <div style={{ fontSize: '0.6rem', color: '#64748b' }}>Distance</div>
                      <div style={{ fontSize: '1.2rem', fontWeight: '700', color: '#1e293b' }}>{result.distance} km</div>
                    </div>
                    <div style={{ textAlign: 'center', background: '#f8fafc', padding: '16px', borderRadius: '12px' }}>
                      <div style={{ fontSize: '0.6rem', color: '#64748b' }}>Time</div>
                      <div style={{ fontSize: '1.2rem', fontWeight: '700', color: '#1e293b' }}>{result.time} min</div>
                    </div>
                  </div>
                  <div style={{ marginTop: '12px', padding: '12px', background: emeraldLight, borderRadius: '10px' }}>
                    <div style={{ fontSize: '0.8rem', color: '#1e293b' }}>💡 10km race time: <strong>{(result.paceMinutes * 10 + (result.paceSeconds / 60) * 10).toFixed(1)}</strong> min</div>
                  </div>
                </div>
              ) : (
                <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '300px' }}>
                  ...
                  <p style={{ color: '#94a3b8', textAlign: 'center' }}>Enter distance and time to calculate your pace</p>
                </div>
              )}
            </div>
          </div>
