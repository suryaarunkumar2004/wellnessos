...
export default function CalorieBurnEstimator() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('calorie_burn_history');
    if (saved) setHistory(JSON.parse(saved));
  }, []);

  useEffect(() => {
    if (history.length > 0) localStorage.setItem('calorie_burn_history', JSON.stringify(history));
  }, [history]);
  ...
  const calculateCalories = () => {
    ...
    const calories = met * w * (d / 60) * 1.05;
    ...
    localStorage.setItem('calorie_burn_history', JSON.stringify(history));
    showToast('✅ Calories estimated!', 'success');
  };

  const resetCalculator = () => { setDuration(''); setWeight(''); setResult(null); showToast('Reset', 'info'); };
  const clearHistory = () => { setHistory([]); localStorage.removeItem('calorie_burn_history'); showToast('History cleared', 'info'); };
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 24px' }}>
          ...
          <div style={{ background: 'linear-gradient(135deg, #059669 0%, #047857 50%, #065f46 100%)', borderRadius: '16px', padding: '32px 36px', marginBottom: '32px', position: 'relative', overflow: 'hidden', color: 'white', boxShadow: '0 4px 20px rgba(5, 150, 105, 0.25)' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div>
                  <h1 style={{ fontSize: '1.8rem', fontWeight: '800', margin: 0, letterSpacing: '-0.5px' }}>Calorie Burn Estimator</h1>
                  <p style={{ fontSize: '0.9rem', opacity: 0.9, margin: '2px 0 0', fontWeight: '300' }}>MET-based burn rate by exercise & weight</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px' }}>
            <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: '1px solid #e2e8f0' }}>
              <h3 style={{ fontSize: '1.2rem', fontWeight: '800', color: '#1e293b', marginBottom: '20px' }}>Enter Your Workout</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                  {Object.keys(metValues).map((act) => (
                    <button key={act} onClick={() => setActivity(act)} style={{ padding: '8px', borderRadius: '10px', border: `2px solid ${activity === act ? emerald : '#e2e8f0'}`, background: activity === act ? emeraldLight : 'white', color: activity === act ? emerald : '#64748b', fontWeight: '700', cursor: 'pointer', fontSize: '0.75rem' }}>
                      {getActivityLabel(act)}
                    </button>
                  ))}
                </div>
                <input type="number" value={duration} onChange={e => setDuration(e.target.value)} placeholder="Duration (minutes) *" style={{ width: '100%', padding: '12px 16px', borderRadius: '12px', border: '1.5px solid #e2e8f0', fontSize: '1rem', outline: 'none' }} />
                ...
                <div style={{ display: 'flex', gap: '12px' }}>
                  <button onClick={calculateCalories} style={{ flex: 2, padding: '14px', background: emerald, color: 'white', border: 'none', borderRadius: '12px', fontWeight: '700', fontSize: '1rem', cursor: 'pointer' }}>Calculate</button>
            ...
            <div>
              {result ? (
                <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: `2px solid ${emerald}` }}>
                  <h3 style={{ fontSize: '1.2rem', fontWeight: '800', color: '#1e293b', marginBottom: '16px' }}>Calories Burned</h3>
                  <div style={{ textAlign: 'center', marginBottom: '16px' }}>
                    <div style={{ fontSize: '3rem', fontWeight: '800', color: '#ef4444' }}>{result.calories}</div>
                    <div style={{ fontSize: '1rem', color: '#64748b' }}>calories</div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                    <div style={{ textAlign: 'center', background: '#f8fafc', padding: '12px', borderRadius: '10px' }}>
                      <div style={{ fontSize: '0.6rem', color: '#64748b' }}>Activity</div>
                      <div style={{ fontSize: '0.9rem', fontWeight: '700', color: '#1e293b' }}>{getActivityLabel(result.activity)}</div>
                    </div>
                    <div style={{ textAlign: 'center', background: '#f8fafc', padding: '12px', borderRadius: '10px' }}>
                      <div style={{ fontSize: '0.6rem', color: '#64748b' }}>Duration</div>
                      <div style={{ fontSize: '0.9rem', fontWeight: '700', color: '#1e293b' }}>{result.duration} min</div>
                    </div>
                    <div style={{ textAlign: 'center', background: '#f8fafc', padding: '12px', borderRadius: '10px' }}>
                      <div style={{ fontSize: '0.6rem', color: '#64748b' }}>Weight</div>
                      <div style={{ fontSize: '0.9rem', fontWeight: '700', color: '#1e293b' }}>{result.weight} kg</div>
                    </div>
                  </div>
                  <div style={{ marginTop: '12px', padding: '12px', background: '#fef2f2', borderRadius: '10px' }}>
                    <div style={{ fontSize: '0.8rem', color: '#64748b' }}>�� Equivalent to walking <strong>{Math.round(result.calories / 3)}</strong> minutes!</div>
                  </div>
                </div>
              ) : (
                <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '300px' }}>
                  ...
                  <p style={{ color: '#94a3b8', textAlign: 'center' }}>Enter your workout details</p>
                </div>
              )}
            </div>
          </div>
