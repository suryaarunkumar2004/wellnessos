...
export default function CalorieCalculator() {
  ...
  useEffect(() => {
    if (history.length > 0) localStorage.setItem('calorie_history', JSON.stringify(history));
  }, [history]);
  ...
  const clearHistory = () => { setHistory([]); localStorage.removeItem('calorie_history'); showToast('History cleared', 'info'); };
          ...
          <div style={{ background: 'linear-gradient(135deg, #059669 0%, #047857 50%, #065f46 100%)', borderRadius: '16px', padding: '32px 36px', marginBottom: '32px', position: 'relative', overflow: 'hidden', color: 'white', boxShadow: '0 4px 20px rgba(5, 150, 105, 0.25)' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div>
                  <h1 style={{ fontSize: '1.8rem', fontWeight: '800', margin: 0, letterSpacing: '-0.5px' }}>Calorie Calculator</h1>
                  <p style={{ fontSize: '0.9rem', opacity: 0.9, margin: '2px 0 0', fontWeight: '300' }}>Calculate your daily calorie needs</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div>
              {result ? (
                <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: `2px solid ${emerald}` }}>
                  ...
                  <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '10px', marginTop: '12px' }}>
                    <div style={{ fontSize: '0.8rem', color: '#64748b' }}>Activity: <strong>{result.activity}</strong></div>
                  </div>
                </div>
              ) : (
                <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '300px' }}>
                  ...
                  <p style={{ color: '#94a3b8', textAlign: 'center' }}>Enter your details to calculate calories</p>
                </div>
              )}
            </div>
          </div>
