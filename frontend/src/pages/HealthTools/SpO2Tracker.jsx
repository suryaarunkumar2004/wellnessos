...
import { FaArrowLeft, FaLungs, FaHistory, FaDownload, FaPrint, FaShare, FaTrash } from 'react-icons/fa';

export default function SpO2Tracker() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('spo2_logs');
    if (saved) setLogs(JSON.parse(saved));
  }, []);

  useEffect(() => {
    if (logs.length > 0) localStorage.setItem('spo2_logs', JSON.stringify(logs));
  }, [logs]);
  ...
  const logSpO2 = () => {
    ...
    if (s >= 95) { status = 'Normal'; color = '#22c55e'; advice = 'Your oxygen saturation is normal. Excellent!'; }
    else if (s >= 92) { status = 'Below Normal'; color = '#f59e0b'; advice = 'Your oxygen level is slightly low. Monitor regularly.'; }
    else if (s >= 88) { status = 'Low'; color = '#f97316'; advice = 'Your oxygen level is low. Consult a healthcare provider.'; }
    else { status = 'Critical'; color = '#ef4444'; advice = 'Your oxygen level is critically low. Seek immediate medical attention!'; }
    ...
    setResult(logEntry);
    showToast('SpO2 logged! ✅', 'success');
  };

  const clearLogs = () => { setLogs([]); localStorage.removeItem('spo2_logs'); showToast('Logs cleared', 'info'); };
  const getAvgSpO2 = () => logs.length ? Math.round(logs.reduce((sum, l) => sum + l.spo2, 0) / logs.length) : 0;
          ...
          <div style={{ background: 'linear-gradient(135deg, #059669 0%, #047857 50%, #065f46 100%)', borderRadius: '16px', padding: '32px 36px', marginBottom: '32px', position: 'relative', overflow: 'hidden', color: 'white', boxShadow: '0 4px 20px rgba(5, 150, 105, 0.25)' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div>
                  <h1 style={{ fontSize: '1.8rem', fontWeight: '800', margin: 0, letterSpacing: '-0.5px' }}>SpO2 Tracker</h1>
                  <p style={{ fontSize: '0.9rem', opacity: 0.9, margin: '2px 0 0', fontWeight: '300' }}>Monitor your blood oxygen levels</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px' }}>
            <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: '1px solid #e2e8f0' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <input type="number" value={spo2} onChange={e => setSpo2(e.target.value)} placeholder="Oxygen Saturation (%) *" style={{ width: '100%', padding: '12px 16px', borderRadius: '12px', border: '1.5px solid #e2e8f0', fontSize: '1rem', outline: 'none' }} />
                <input type="number" value={heartRate} onChange={e => setHeartRate(e.target.value)} placeholder="Heart Rate (optional)" style={{ width: '100%', padding: '12px 16px', borderRadius: '12px', border: '1.5px solid #e2e8f0', fontSize: '1rem', outline: 'none' }} />
                <div style={{ display: 'flex', gap: '12px' }}>
                  <button onClick={logSpO2} style={{ flex: 2, padding: '14px', background: emerald, color: 'white', border: 'none', borderRadius: '12px', fontWeight: '700', fontSize: '1rem', cursor: 'pointer' }}>Log Reading</button>
              ...
              {logs.length > 0 ? (
                <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: `2px solid ${emerald}` }}>
                  ...
                  <div style={{ textAlign: 'center', marginBottom: '16px' }}>
                    <div style={{ fontSize: '2.5rem', fontWeight: '800', color: emerald }}>{getAvgSpO2()}%</div>
                    <div style={{ fontSize: '0.8rem', color: '#64748b' }}>Average Oxygen Level</div>
                  </div>
                  <div style={{ maxHeight: '200px', overflowY: 'auto' }}>
                    {logs.slice(0, 10).map((log, idx) => (
                      <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #f1f5f9' }}>
                        ...
                        <span style={{ fontSize: '0.9rem', fontWeight: '700', color: log.color }}>{log.spo2}%</span>
                        {log.heartRate && <span style={{ fontSize: '0.8rem', color: '#94a3b8' }}>❤️ {log.heartRate}</span>}
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '300px' }}>
                  ...
                  <p style={{ color: '#94a3b8', textAlign: 'center' }}>Start tracking your oxygen levels</p>
                </div>
              )}
            </div>
          </div>
