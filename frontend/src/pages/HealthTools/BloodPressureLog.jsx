...
export default function BloodPressureLog() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('bp_logs');
    if (saved) setLogs(JSON.parse(saved));
  }, []);

  useEffect(() => {
    if (logs.length > 0) localStorage.setItem('bp_logs', JSON.stringify(logs));
  }, [logs]);
  ...
  const logBloodPressure = () => {
    ...
    if (sys < 120 && dia < 80) {
      category = 'Normal';
      color = '#22c55e';
      advice = 'Your blood pressure is normal. Keep up the healthy lifestyle!';
    } else if (sys < 130 && dia < 80) {
      category = 'Elevated';
      color = '#f59e0b';
      advice = 'Your blood pressure is elevated. Monitor regularly.';
    } else if (sys < 140 || dia < 90) {
      category = 'Stage 1 Hypertension';
      color = '#f97316';
      advice = 'You may have stage 1 hypertension. Consult your doctor.';
    } else if (sys < 180 || dia < 120) {
      category = 'Stage 2 Hypertension';
      color = '#ef4444';
      advice = 'You may have stage 2 hypertension. Consult your doctor immediately.';
    } else {
    ...
    setResult(logEntry);
    showToast('Blood pressure logged! ✅', 'success');
  };

  const clearLogs = () => { setLogs([]); localStorage.removeItem('bp_logs'); showToast('Logs cleared', 'info'); };
  const getAvgSystolic = () => logs.length ? Math.round(logs.reduce((sum, l) => sum + l.systolic, 0) / logs.length) : 0;
  const getAvgDiastolic = () => logs.length ? Math.round(logs.reduce((sum, l) => sum + l.diastolic, 0) / logs.length) : 0;
          ...
          <div style={{ background: 'linear-gradient(135deg, #059669 0%, #047857 50%, #065f46 100%)', borderRadius: '16px', padding: '32px 36px', marginBottom: '32px', position: 'relative', overflow: 'hidden', color: 'white', boxShadow: '0 4px 20px rgba(5, 150, 105, 0.25)' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div>
                  <h1 style={{ fontSize: '1.8rem', fontWeight: '800', margin: 0, letterSpacing: '-0.5px' }}>Blood Pressure Log</h1>
                  <p style={{ fontSize: '0.9rem', opacity: 0.9, margin: '2px 0 0', fontWeight: '300' }}>Track and monitor your blood pressure</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: '1px solid #e2e8f0' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div style={{ display: 'flex', gap: '12px' }}>
                  <button onClick={logBloodPressure} style={{ flex: 2, padding: '14px', background: emerald, color: 'white', border: 'none', borderRadius: '12px', fontWeight: '700', fontSize: '1rem', cursor: 'pointer' }}>Log Reading</button>
            ...
            <div>
              {logs.length > 0 ? (
                <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: `2px solid ${emerald}` }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
                    <div style={{ textAlign: 'center', background: emeraldLight, padding: '12px', borderRadius: '10px' }}>
                      <div style={{ fontSize: '0.6rem', color: '#64748b' }}>Avg Systolic</div>
                      <div style={{ fontSize: '1.5rem', fontWeight: '800', color: emerald }}>{getAvgSystolic()}</div>
                    </div>
                    <div style={{ textAlign: 'center', background: '#fef3c7', padding: '12px', borderRadius: '10px' }}>
                      <div style={{ fontSize: '0.6rem', color: '#64748b' }}>Avg Diastolic</div>
                      <div style={{ fontSize: '1.5rem', fontWeight: '800', color: '#d97706' }}>{getAvgDiastolic()}</div>
                    </div>
                  </div>
                  <div style={{ maxHeight: '200px', overflowY: 'auto' }}>
                    {logs.slice(0, 10).map((log, idx) => (
                      <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #f1f5f9' }}>
                        ...
                        <span style={{ fontSize: '0.9rem', fontWeight: '700', color: log.color }}>{log.systolic}/{log.diastolic}</span>
                        <span style={{ fontSize: '0.7rem', color: log.color }}>{log.category}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '300px' }}>
                  ...
                  <p style={{ color: '#94a3b8', textAlign: 'center' }}>Start tracking your blood pressure</p>
                </div>
              )}
            </div>
          </div>
