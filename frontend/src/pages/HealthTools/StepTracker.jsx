...
export default function StepTracker() {
  ...
  useEffect(() => {
    if (logs.length > 0) localStorage.setItem('step_logs', JSON.stringify(logs));
  }, [logs]);
  ...
  const logSteps = () => {
    if (!steps || !date) {
      showToast('Please enter steps and date', 'error');
      return;
    }
    const s = parseInt(steps);
    const existing = logs.findIndex(l => l.date === date);
    const logEntry = { date, steps: s, timestamp: new Date().toISOString() };
    let updatedLogs;
    if (existing !== -1) {
      updatedLogs = [...logs];
      updatedLogs[existing] = logEntry;
    } else {
      updatedLogs = [logEntry, ...logs];
    }
    setLogs(updatedLogs);
    setResult(logEntry);
  ...
  const clearLogs = () => { setLogs([]); localStorage.removeItem('step_logs'); showToast('Logs cleared', 'info'); };

  const getTotalSteps = () => logs.reduce((sum, l) => sum + l.steps, 0);
  const getAverageSteps = () => logs.length ? Math.round(getTotalSteps() / logs.length) : 0;
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 24px' }}>
          ...
          <div style={{ background: 'linear-gradient(135deg, #059669 0%, #047857 50%, #065f46 100%)', borderRadius: '16px', padding: '32px 36px', marginBottom: '32px', position: 'relative', overflow: 'hidden', color: 'white', boxShadow: '0 4px 20px rgba(5, 150, 105, 0.25)' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div>
                  <h1 style={{ fontSize: '1.8rem', fontWeight: '800', margin: 0, letterSpacing: '-0.5px' }}>Step Tracker</h1>
                  <p style={{ fontSize: '0.9rem', opacity: 0.9, margin: '2px 0 0', fontWeight: '300' }}>Track your daily steps and progress</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px' }}>
            <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: '1px solid #e2e8f0' }}>
              <h3 style={{ fontSize: '1.2rem', fontWeight: '800', color: '#1e293b', marginBottom: '20px' }}>Log Your Steps</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <input type="number" value={steps} onChange={e => setSteps(e.target.value)} placeholder="Steps *" style={{ width: '100%', padding: '12px 16px', borderRadius: '12px', border: '1.5px solid #e2e8f0', fontSize: '1rem', outline: 'none' }} />
                <div style={{ display: 'flex', gap: '12px' }}>
                  <button onClick={logSteps} style={{ flex: 2, padding: '14px', background: emerald, color: 'white', border: 'none', borderRadius: '12px', fontWeight: '700', fontSize: '1rem', cursor: 'pointer' }}>Log Steps</button>
                  <button onClick={clearLogs} style={{ flex: 1, padding: '14px', background: '#fef2f2', color: '#ef4444', border: 'none', borderRadius: '12px', fontWeight: '600', fontSize: '1rem', cursor: 'pointer' }}>Clear</button>
                </div>
              </div>
            </div>
            <div>
              {logs.length > 0 ? (
                <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: `2px solid ${emerald}` }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
                    <div style={{ textAlign: 'center', background: emeraldLight, padding: '12px', borderRadius: '10px' }}>
                      ...
                      <div style={{ fontSize: '1.5rem', fontWeight: '800', color: emerald }}>{getTotalSteps().toLocaleString()}</div>
                    </div>
                    <div style={{ textAlign: 'center', background: '#fef3c7', padding: '12px', borderRadius: '10px' }}>
                      ...
                      <div style={{ fontSize: '1.5rem', fontWeight: '800', color: '#d97706' }}>{getAverageSteps().toLocaleString()}</div>
                    </div>
                  </div>
                  <div style={{ maxHeight: '200px', overflowY: 'auto' }}>
                    {logs.slice(0, 10).map((log, idx) => (
                      <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #f1f5f9' }}>
                        ...
                        <span style={{ fontSize: '0.9rem', fontWeight: '700', color: emerald }}>{log.steps.toLocaleString()}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ background: 'white', borderRadius: '20px', padding: '32px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '300px' }}>
                  ...
                  <p style={{ color: '#94a3b8', textAlign: 'center' }}>Start tracking your daily steps</p>
                </div>
              )}
            </div>
          </div>
